...
<Image
  src={import('./images/system-actions.png')}
  alt="All the System actions available in Vyuh"
  width={[350]}
/>
...
between two ore more actions based on some runtime condition.
...
in the code below. The use of a static **`typeDescriptor`** has become a common
...
<Aside>
  One fun fact to note is that the **UPC**, **EAN** and **ISBN** formats can
  only encode numbers, so it will fail for our string "Vyuh Framework" :hushed:.
</Aside>

<YouTube
  id={'https://youtu.be/evt-9izEp-s'}
  title={'Quick test of the Show Barcode action'}
/>
