...
- Sets up content type descriptors for your custom models. By default, it sets
  up the `vyuh.document` content type for `Document` models and the system
  content types in the
  [`vyuh_feature_system`](/framework/packages/vyuh-feature-system) package.
  These can later be used for rendering your content.
