...
<LinkCard
  href={'/guides/cms/custom-content-type'}
...
<Aside title={'ContentDescriptor & ContentSchemaBuilder'}>
...
```typescript title="product-card.ts"
...
<Aside>
  The **`defaultLayoutConfiguration`** is just a convenience method that creates
  a schema for a default layout. It uses the schemaType of the content as a
  prefix. Thus the for the `ProductCard`, it would be
  **`misc.productCard.layout.default`**.
</Aside>
