...
<Aside>
  A good practice is to include the possible values of this condition at
  runtime, which we do in the **`description`** field and also in the
  **`preview`**.
</Aside>
...
<Aside title={'Naming inside the Schema'}>
...
In general, we recommend following a dotted notation (`feature.type.name`) for
naming schemas.
...
```dart title="lib/condition/part_of_day.dart"
