...
possible enumerated values: `false` and `true`.
...
custom conditions in its [dedicated guide](/guides/cms/custom-condition).

## Where can a condition be used?

We already saw one instance where a condition could be used: Actions! However,
they are applicable in all the following areas:
