---
title: Actions
description: Understanding the structure and usage of actions in the Vyuh React feature system
---
...
Actions in the Vyuh framework provide a powerful way to add interactivity to your application. They define what happens when users interact with [content blocks](/react/guides/content-blocks), such as clicking a button, submitting a form, or navigating between screens.
...
const action = {
  ...
  configurations: [
    ...
    { _type: "vyuh.action.openUrl", url: "https://example.com", isAwaited: true }
  ]
};
```
...
<Aside type="tip">
  Use `isAwaited: true` for operations that must complete before subsequent operations can proceed, such as data validation or saving. Use `isAwaited: false` for operations that can run independently, such as analytics tracking or UI updates.
</Aside>

## Conditional Actions

The Vyuh framework also supports conditional actions, which allow you to dynamically choose which action to execute based on a [condition](/react/guides/conditions):
