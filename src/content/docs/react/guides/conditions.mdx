---
title: Conditions
description: Understanding the structure and usage of conditions in the Vyuh React feature system
---
...
Conditions in the Vyuh framework provide a powerful way to create dynamic, context-aware [content](/react/guides/content-blocks) and interactions. They allow your application to make decisions based on various factors such as user state, device characteristics, time, or any custom logic you define.
...
Conditions are most commonly used with [conditional content blocks](/react/guides/content-blocks#conditional-content) to dynamically select which content to display:
...
Conditions can also be used with [conditional actions](/react/guides/actions#conditional-actions) to dynamically select which action to execute:
...
<Aside type="caution">
  Unlike actions which can have multiple configurations, a condition must have exactly one configuration. This is enforced in the schema with a validation rule: `Rule.max(1)`.
</Aside>
...
Now that you understand conditions in Vyuh, learn about:
...
<LinkCard
  ...
  description="Explore the various content blocks available in the Vyuh React feature system."
  href="/react/content-blocks"
/>
